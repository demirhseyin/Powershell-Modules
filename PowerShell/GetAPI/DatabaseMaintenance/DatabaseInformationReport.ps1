...
foreach ($instance in get-content $ComputerList)
{
     ...
     $Sheet.Cells.Item($intRow,3) = "Compatibilty Level"
     ...
     $Sheet.Cells.Item($intRow,8) = "Last Backup Date"
     $Sheet.Cells.Item($intRow,9) = "Is Update Required"
     ...
     ForEach ($db in $dbs) 
     {
          ...
          $Sheet.Cells.Item($intRow, 8) = $db.LastBackupDate 
          $Sheet.Cells.Item($intRow, 9) = $db.IsUpdateable
